...
# @flag -i --interactive Choose extension interactively, then choose artifacts from extension or site.
...
Export all discovered Afform, SavedSearch, and SearchDisplay artifacts:
  export-searchkit scan --ext /var/www/html/ext/some-ext-name

Export and continue if one item fails:
  export-searchkit scan --no-strict --ext /var/www/html/ext/some-ext-name

Export one Afform:
  export-searchkit export_items --ext /var/www/html/ext/some-ext-name afform:afsearchManageEventTemplates

Export SavedSearch/SearchDisplay by ID:
  export-searchkit export_items --ext /var/www/html/ext/some-ext-name ss:65 ss:66 sd:67 sd:68

Dry-run scan:
  export-searchkit --dry-run scan --ext /var/www/html/ext/some-ext-name
  export-searchkit scan --dry-run --ext /var/www/html/ext/some-ext-name

Dry-run explicit export:
  export-searchkit --dry-run export_items --ext /var/www/html/ext/some-ext-name \
    afform:afsearchManageEventTemplates ss:65 sd:67
  export-searchkit export_items --dry-run --ext /var/www/html/ext/some-ext-name \
    afform:afsearchManageEventTemplates ss:65 sd:67

Debug export and keep temp logs:
  export-searchkit --debug --keep-tmp scan --ext /var/www/html/ext/some-ext-name

Trace shell execution:
  export-searchkit --trace --keep-tmp scan --ext /var/www/html/ext/some-ext-name
...
Run built-in smoke tests:
  export-searchkit self_test --ext /var/www/html/ext/some-ext-name
...
self_test() {
  ...
  resolved_ext="$(awk -F': ' '/^Extension:/ {print $2; exit}' "$out")"
  [[ -n "$resolved_ext" ]] || die "self-test failed: could not read resolved extension path from doctor output"
  ...
  local ext_choice
  ext_choice="$(interactive_choice_for_path "$resolved_ext")"
...
Use --ext with a CiviCRM extension path, for example:
  export-searchkit doctor --ext sites/default/civicrm-extend/com.example.ext
  export-searchkit doctor --ext web/sites/default/civicrm-extend/com.example.ext
  export-searchkit doctor --ext wp-content/civicrm-extend/com.example.ext
  export-searchkit doctor --ext web/wp-content/civicrm-extend/com.example.ext
  export-searchkit interactive"
...
find_ext_path_by_key() {
  ...
  for d in \
    ...
    "$project_root/extensions/$key" \
    "$project_root/extensions/${key//_/-}" \
    "$project_root/civicrm/ext/$key" \
    "$project_root/civicrm/ext/${key//_/-}" \
    "$project_root/private/civicrm/ext/$key" \
    "$project_root/private/civicrm/ext/${key//_/-}" \
    ...
    "$project_root/web/sites/default/civicrm-extend/$key" \
    "$project_root/web/sites/default/civicrm-extend/${key//_/-}" \
    "$project_root/sites/default/civicrm-extend/$key" \
    "$project_root/sites/default/civicrm-extend/${key//_/-}" \
    "$project_root/web/wp-content/civicrm-extend/$key" \
    "$project_root/web/wp-content/civicrm-extend/${key//_/-}" \
    "$project_root/wp-content/civicrm-extend/$key" \
    "$project_root/wp-content/civicrm-extend/${key//_/-}" \
    "$project_root/web/wp-content/plugins/$key" \
    "$project_root/web/wp-content/plugins/${key//_/-}" \
    "$project_root/wp-content/plugins/$key" \
    "$project_root/wp-content/plugins/${key//_/-}" \
    "$parent_root/ext/$key" \
    "$parent_root/ext/${key//_/-}" \
    "$parent_root/extensions/$key" \
    "$parent_root/extensions/${key//_/-}" \
    "$parent_root/web/sites/default/civicrm-extend/$key" \
    "$parent_root/web/sites/default/civicrm-extend/${key//_/-}" \
    "$parent_root/sites/default/civicrm-extend/$key" \
    "$parent_root/sites/default/civicrm-extend/${key//_/-}" \
    "$parent_root/web/wp-content/civicrm-extend/$key" \
    "$parent_root/web/wp-content/civicrm-extend/${key//_/-}" \
    "$parent_root/wp-content/civicrm-extend/$key" \
    "$parent_root/wp-content/civicrm-extend/${key//_/-}"
  do
    [[ -d "$d" ]] && {
      printf '%s\n' "$d"
      return 0
  ...
  while IFS= read -r d; do
    ...
    find "$project_root" "$parent_root" \
      -path '*/vendor' -prune -o \
      -path '*/node_modules' -prune -o \
      -path '*/.git' -prune -o \
      -path '*/wp-content/uploads' -prune -o \
      -path '*/sites/default/files' -prune -o \
      -name info.xml -print 2>/dev/null \
      | sed 's#/info.xml$##'
  )
...
list_filesystem_extensions() {
  ...
  configured_dir="$(
    "$CV_BIN" php:eval '
      $dirs = Civi::settings()->get("extensionsDir");
...

  {
    ...
    printf '%s\n' \
      ...
      "$project_root/civicrm/ext" \
      "$project_root/private/civicrm/ext" \
      "$project_root/web/sites/default/files/civicrm/ext" \
      "$project_root/sites/default/files/civicrm/ext" \
      "$project_root/web/sites/default/civicrm-extend" \
      "$project_root/sites/default/civicrm-extend" \
      "$project_root/web/modules/custom" \
      "$project_root/web/modules/contrib" \
      "$project_root/modules/custom" \
      "$project_root/modules/contrib" \
      "$project_root/web/wp-content/civicrm-extend" \
      "$project_root/wp-content/civicrm-extend" \
      "$project_root/web/wp-content/plugins" \
      "$project_root/wp-content/plugins" \
      "$parent_root/ext" \
      "$parent_root/extensions" \
      "$parent_root/civicrm/ext" \
      "$parent_root/private/civicrm/ext" \
      "$parent_root/web/sites/default/files/civicrm/ext" \
      "$parent_root/sites/default/files/civicrm/ext" \
      "$parent_root/web/sites/default/civicrm-extend" \
      "$parent_root/sites/default/civicrm-extend" \
      "$parent_root/web/wp-content/civicrm-extend" \
      "$parent_root/wp-content/civicrm-extend" \
      "$parent_root/web/wp-content/plugins" \
      "$parent_root/wp-content/plugins"
    ...
    find "$project_root" "$parent_root" \
      ...
      -name info.xml -print 2>/dev/null \
      | sed 's#/info.xml$##'
  } | sed '/^[[:space:]]*$/d' | sort -u | while IFS= read -r ext_dir; do
    [[ -d "$ext_dir" ]] || continue
...
choose_ext_interactive() {
  ...
  [[ -s "$paths_file" ]] || {
    ...
    err ""
    err "  export-searchkit scan --ext /var/www/html/ext/some-ext-name"
    err "  export-searchkit list --ext /var/www/html/ext/some-ext-name"
    err ""
